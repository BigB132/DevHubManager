...
module.exports = async (client, interaction) => {
    if(interaction.isStringSelectMenu) {
        if(interaction.customId === "assignMTCDD") {
            const query = {
            ...
            const embed = new EmbedBuilder()
                .setTitle("Choose the Channel you want to assign the User to")
            ...
            const assignMTCDD2 = new StringSelectMenuBuilder()
                .setCustomId("assignMTCDD2" + "+" + userId)
                .setMinValues(1)
                .setMaxValues(1)
            ...
            for (let i = 0; i < projectData.channelIds.length; i++) {
                ...
                assignMTCDD2.addOptions({
                    label: channel.name,
                    value: channel.id,
                })
            };

            const actionRow = new ActionRowBuilder().addComponents(assignMTCDD2);
